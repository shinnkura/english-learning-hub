...
import { LogIn, UserPlus, Mail, Lock, ArrowRight, Loader2, ArrowLeft } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'signup') {
        ...
        if (resetEmailSent) {
          setError(`セキュリティのため、${resetTimeout}秒後に再度送信できます。`);
          return;
        }

        await resetPassword(email);
        setError('パスワード再設定用のメールを送信しました。メールを確認してください。');
        ...
        <div className="mt-4">
          {error && (
            <div className={`p-4 rounded-lg mb-4 ${
              error.includes('確認メール') || error.includes('再設定') || error.includes('セキュリティ')
            ...
            <div className="space-y-4">
              ...
              >
                {isLoading ? (
                  ...
                  <>
                    {mode === 'signup' ? 'アカウントを作成' : mode === 'reset' ? 'メールを送信' : 'ログイン'}
              ...
              <div className="space-y-2">
                {mode === 'reset' ? (
                  ...
                  <>
                    ...
                    >
                      {mode === 'login' ? 'アカウントをお持ちでない方' : 'すでにアカウントをお持ちの方'}は
