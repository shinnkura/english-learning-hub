    ...
    const fetchActivityData = async () => {
      try {
        ...
        const activityData = days.map(({ date, start, end }) => {
          const dayWords = wordsResult.data?.filter(w => 
            w.created_at >= start && w.created_at <= end
          ).length || 0;

          const dayVideos = new Set(
            videosResult.data?.filter(v => 
              v.created_at >= start && v.created_at <= end
            ).map(v => v.video_id)
          ).size;

          const dayReviews = reviewsResult.data?.filter(r =>
  ...
  return (
    <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-xl p-4 sm:p-6">
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-6">
        <h2 className="text-xl font-semibold text-gray-900 dark:text-white">学習アクティビティ</h2>
